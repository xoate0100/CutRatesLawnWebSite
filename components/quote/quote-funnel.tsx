...
export function QuoteFunnel() {
  ...
  function computeEstimate(): EstimateResult | null {
    ...
    const outcome = calculateEstimate({
      ...
      frequency:
        serviceId === "mowing" || serviceId === "full-service" || serviceId === "residential"
          ? ((values.frequency as Frequency) || "weekly")
          : "weekly",
  ...
  async function postLead(status: "complete" | "partial") {
    ...
    if (estimable && quote) {
      ...
      if (serviceId === "mowing" || serviceId === "full-service" || serviceId === "residential") {
